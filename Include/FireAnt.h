...
function  ElapsedTime(Exchange) { 
	TimeElapse = Now(4);
	MinuteElapsed = floor(TimeElapse /10000)*60 + floor((TimeElapse - floor(TimeElapse /10000) *10000)/100) - 540; 

	if (Exchange == "HSX") {
		if (MinuteElapsed >0 AND MinuteElapsed <=15) {
			RelativeTimeElapsed = 0;
		}
		if (MinuteElapsed >15 AND MinuteElapsed <=150) {
		...
		else if (MinuteElapsed >345 AND MinuteElapsed <=360) {
			RelativeTimeElapsed = MinuteElapsed - 90;
		}
		else {
			RelativeTimeElapsed = 270;
		}
	}
		...
		else if (MinuteElapsed >240 AND MinuteElapsed <=360) {
			RelativeTimeElapsed = MinuteElapsed - 90;
		}
		else {
			RelativeTimeElapsed = 270;
	...
	else { // Future Contracts
		if (MinuteElapsed >0 AND MinuteElapsed <=150) {
			RelativeTimeElapsed = MinuteElapsed+15;
		}
		else if (MinuteElapsed >150 AND MinuteElapsed <=240) {
			RelativeTimeElapsed = 165;
		}
		else if (MinuteElapsed >240 AND MinuteElapsed <=330) {
			RelativeTimeElapsed = MinuteElapsed - 75;
		}
		else if (MinuteElapsed >330 AND MinuteElapsed <=345) {
			RelativeTimeElapsed = 255;
		}
...
function  TransactionSpeed(Exchange) {
	RelativeTimeElapsed = ElapsedTime(Exchange);
	TotalTime = 255;
	if (Exchange == "HSX") 
		TotalTime = 255;
	else 
	...
	Speed = IIf( bi > lvbi-1, V/RelativeTimeElapsed , V/TotalTime );
	
	return Speed;
}

function  ExpectedVolume(Exchange) { 
	TotalTime = 255;
	if (Exchange == "HSX") 
		TotalTime = 255;
	else 
		TotalTime = 270;
	RelativeTimeElapsed = ElapsedTime(Exchange) /TotalTime;
...
function Pressure(PH, PL, PC, PV) {
	...
	PP = Param("Pressure Period ", 20, 5, 200 , 1);
	SP = 5;
	
	vofavg = MA(vofactor, PP);
